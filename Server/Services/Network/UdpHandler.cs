...
namespace Server.Services.Network
{
    internal class UdpHandler : ProtocolHandler<Store>
    {
        ...
        protected override async void Handle(string str)
        {
            ...
            {
                Send(new Transaction { Command = Command.Exception, Data = "Đã xảy ra lỗi." });
                Logger.LogTransaction(req, ex);
            }
        }
        ...
        private async Task SendInviteHandle(Transaction req)
        {
            ...
            // to who sent request
            Send(new Transaction { Command = Command.SendInvite, Data = new { Message = "Đã gửi lời mời kết bạn" } });
        ...
        private void SignInHandle(Transaction req)
        {
            ...
            if (user == null)
            {
                Send(new Transaction { Command = Command.SignIn, Data = new { IsOk = false, Message = "Không tìm thấy người dùng" } });
                return;
            }

            if (!Hasher.Verify(password, user.Password))
            {
                Send(new Transaction { Command = Command.SignIn, Data = new { IsOk = false, Message = "Sai mật khẩu" } });
                return;
            }
